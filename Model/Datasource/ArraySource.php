...
/**
	...
	* @param Model model
	*/
	public function query($method, $queryData = array(), Model $model){
		$condition = array_shift($queryData);
		$field = $type = $limit = null;
		if(strpos($method,'findBy') === 0){
			$type = 'first';
			$field = Inflector::underscore((str_replace('findBy','',$method)));
			$limit = 1;
		} elseif(strpos($method,'findAllBy') === 0){
			$type = 'all';
			$field = Inflector::underscore((str_replace('findAllBy','',$method)));
		}
		if($field && $condition && $type){
			return $model->find($type, array(
				'conditions' => array(
					"{$model->alias}.$field" => $condition
				),
				'limit' => $limit
			));
		}
